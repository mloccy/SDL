...
macro(CheckX11)
  ...
  if(SDL_X11)
    ...
    set(Xau_PKG_CONFIG_SPEC xau)
    set(Xcb_PKG_CONFIG_SPEC xcb)
    ...
    if(PKG_CONFIG_FOUND)
      pkg_check_modules(PC_XAU IMPORTED_TARGET QUIET ${Xau_PKG_CONFIG_SPEC})
      pkg_check_modules(PC_XCB IMPORTED_TARGET QUIET ${Xcb_PKG_CONFIG_SPEC})
      target_link_libraries(PkgConfig::PC_XCB INTERFACE PkgConfig::PC_XAU)
    endif()

    set(X11_EXTRA_LINK_OPTIONS -lXau -lxcb)
    find_package(x11 CONFIG REQUIRED)

    foreach(_LIB X11 Xext Xcursor Xi Xfixes Xrandr Xrender Xss Xtst Xau xcb)
      ...
      if(SDL_X11_SHARED)
        ...
        if(X11_LIB)
          if(HAVE_X11_SHARED)
            ...
            sdl_link_dependency(x11 LIBS PkgConfig::x11 CMAKE_MODULE x11 PKG_CONFIG_SPECS ${X11_PKG_CONFIG_SPEC} HUNTER_MODULE)
            sdl_link_dependency(xcb LIBS PkgConfig::PC_XCB PKG_CONFIG_PREFIX PC_XCB LINK_OPTIONS ${X11_EXTRA_LINK_OPTIONS} PKG_CONFIG_LINK_OPTIONS ${X11_EXTRA_LINK_OPTIONS} PKG_CONFIG_SPECS ${Xcb_PKG_CONFIG_SPEC})
            sdl_link_dependency(xau PKG_CONFIG_PREFIX PC_XAU PKG_CONFIG_SPECS ${Xau_PKG_CONFIG_SPEC})

            sdl_test_link_dependency(x11 LIBS PkgConfig::x11 CMAKE_MODULE x11 PKG_CONFIG_SPECS ${X11_PKG_CONFIG_SPEC} HUNTER_MODULE)
            sdl_test_link_dependency(xcb LIBS PkgConfig::PC_XCB PKG_CONFIG_PREFIX PC_XCB LINK_OPTIONS ${X11_EXTRA_LINK_OPTIONS} PKG_CONFIG_LINK_OPTIONS ${X11_EXTRA_LINK_OPTIONS} PKG_CONFIG_SPECS ${Xcb_PKG_CONFIG_SPEC})
            sdl_test_link_dependency(xau PKG_CONFIG_PREFIX PC_XAU PKG_CONFIG_SPECS ${Xau_PKG_CONFIG_SPEC})
        ...
        if(XEXT_LIB)
          if(HAVE_X11_SHARED)
            ...
            sdl_link_dependency(xext LIBS PkgConfig::Xext CMAKE_MODULE xext PKG_CONFIG_SPECS ${Xext_PKG_CONFIG_SPEC})
          endif()
        endif()
      else()
        sdl_link_dependency(x11 LIBS PkgConfig::x11 CMAKE_MODULE x11 PKG_CONFIG_SPECS ${X11_PKG_CONFIG_SPEC} HUNTER_MODULE)
        ...
        sdl_test_link_dependency(x11 LIBS PkgConfig::x11 CMAKE_MODULE x11 PKG_CONFIG_SPECS ${X11_PKG_CONFIG_SPEC} HUNTER_MODULE)
        sdl_test_link_dependency(xcb LIBS PkgConfig::PC_XCB PKG_CONFIG_PREFIX PC_XCB LINK_OPTIONS ${X11_EXTRA_LINK_OPTIONS} PKG_CONFIG_LINK_OPTIONS ${X11_EXTRA_LINK_OPTIONS} PKG_CONFIG_SPECS ${Xcb_PKG_CONFIG_SPEC})
        sdl_test_link_dependency(xau PC_XAU PKG_CONFIG_SPECS ${Xau_PKG_CONFIG_SPEC})
      endif()
      ...
      if(SDL_X11_XCURSOR)
        if (HAVE_XCURSOR_H AND XCURSOR_LIB)
          ...
          if(HAVE_X11_SHARED)
            ...
            sdl_link_dependency(xcursor LIBS PkgConfig::xcursor CMAKE_MODULE xcursor PKG_CONFIG_SPECS ${Xcursor_PKG_CONFIG_SPEC} HUNTER_MODULE)
          endif()
      ...
      if(SDL_X11_XINPUT)
        if(HAVE_XINPUT2_H AND XI_LIB)
          ...
          if(HAVE_X11_SHARED)
            ...
            sdl_link_dependency(xi LIBS PkgConfig::xi CMAKE_MODULE xi PKG_CONFIG_SPECS ${Xi_PKG_CONFIG_SPEC} HUNTER_MODULE)
          endif()
      ...
      if(SDL_X11_XFIXES)
        if (HAVE_XFIXES_H AND HAVE_XINPUT2_H AND XFIXES_LIB)
          if(HAVE_X11_SHARED)
            ...
            find_package(xfixes CONFIG REQUIRED)
            sdl_link_dependency(xfixes LIBS PkgConfig::xfixes CMAKE_MODULE xfixes PKG_CONFIG_SPECS ${Xfixes_PKG_CONFIG_SPEC} HUNTER_MODULE)
          endif()
      ...
      if(SDL_X11_XRANDR)
        if(HAVE_XRANDR_H AND XRANDR_LIB)
          if(HAVE_X11_SHARED)
            ...
            sdl_link_dependency(xrandr LIBS PkgConfig::xrandr CMAKE_MODULE xrandr PKG_CONFIG_SPECS ${Xrandr_PKG_CONFIG_SPEC} HUNTER_MODULE)
          endif()
      ...
      if(SDL_X11_XSCRNSAVER)
        if(HAVE_XSS_H AND XSS_LIB)
          if(HAVE_X11_SHARED)
            ...
            find_package(xscrnsaver CONFIG REQUIRED)
            sdl_link_dependency(xss LIBS PkgConfig::xscrnsaver CMAKE_MODULE xscrnsaver PKG_CONFIG_SPECS ${Xss_PKG_CONFIG_SPEC} HUNTER_MODULE)
          endif()
      ...
      if(SDL_X11_XTEST)
        if(HAVE_XTEST_H AND XTST_LIB)
          if(HAVE_X11_SHARED)
            ...
            find_package(xtst CONFIG REQUIRED)
            sdl_link_dependency(xtst LIBS PkgConfig::xtst CMAKE_MODULE xtst PKG_CONFIG_SPECS ${Xtst_PKG_CONFIG_SPEC} HUNTER_MODULE)
          endif()
